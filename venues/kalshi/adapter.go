...
// v2MarketToKalshiMarket converts a v2 market into KalshiMarket for use with
// AdaptKalshiMarket. The v2 API uses different timestamp names than v1
// (close_time vs close_ts, expected_expiration_time vs expected_expiration_ts)
// so we remap them here.
func v2MarketToKalshiMarket(m V2Market) KalshiMarket {
	return KalshiMarket{
		...
		Volume:               m.Volume,
		CloseTS:              m.CloseTime,
		ExpectedExpirationTS: m.ExpectedExpirationTime,
		OpenTS:               m.OpenTime,
		Status:               m.Status,
		Result:               m.Result,
